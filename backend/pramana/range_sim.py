...
def generate_range(output_dir="."):
    ...
    random.shuffle(handle_pool)

    # how many accounts each operator runs (mostly 2, so positives stay scarce)
    for i in range(N_OPERATORS):
        op_id = f"op_{i:02d}"
        n_acc = random.choices([2, 3, 4], weights=[0.75, 0.15, 0.10])[0]
        compartmented = i < N_COMPARTMENTED          # T5: first few share nothing

        # operator-level signals that SHOULD leak across their accounts
        op_contact = f"05{hexid(4)}"                 # T4: rare contact identifier
        op_template = random.choice(TEMPLATES)
        op_asn = random.choice(ASNS)
        op_pgp = f"PGP_{hexid(8).upper()}"
        op_image = f"img_{i:02d}_{hexid(3)}"
        op_infra_batch = f"batch_{hexid(4)}"         # site-config scrape
        op_cat_batch   = f"batch_{hexid(4)}"         # catalogue scrape (DIFFERENT origin)
        ...
        for j in range(n_acc):
            handle = stem + random.choice(HANDLE_SUFFIX) if j else stem + "_supply"

            if compartmented:
                # T5: disciplined operator - every account looks unrelated
                acct = {
                    "pgp_fp":      HUB_PGP if random.random() < HUB_PGP_RATE else f"PGP_{hexid(8).upper()}",
                    "contact_id":  f"05{hexid(4)}",
                    "asn":         random.choice(ASNS),
                    "template_id": random.choice(TEMPLATES),
                    "infra_batch": f"batch_{hexid(4)}",
                }
                listings = [{"title": random.choice(PRODUCTS),
                             "price_usd": random.choice([80, 120, 240, 495, 900]),
                             "image_id": f"img_{hexid(5)}",
                             "capture_batch": f"batch_{hexid(4)}"}
                            for _ in range(random.randint(3, 5))]
            else:
                # T1: hub PGP may mask the real one
                pgp = HUB_PGP if random.random() < HUB_PGP_RATE else op_pgp
                acct = {
                    "pgp_fp":      pgp,
                ...
                for _ in range(2):
                    listings.append({"title": random.choice(PRODUCTS),
                                     "price_usd": random.choice([80, 120, 240, 495, 900]),
                                     "image_id": op_image,
    ...
    sampled = random.sample(negatives, min(n_neg, len(negatives)))
    ...
    print(f"  {out_dir / 'pairs.csv'}        <- 500 questions with known answers")
    print("=" * 62)
