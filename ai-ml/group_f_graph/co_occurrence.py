...
class CoOccurrenceGraph:
    ...
    def add_document(self, doc_id: str, entities: List[str]) -> None:
        ...
        if doc_id in self.doc_entity_map:
            if self.doc_entity_map[doc_id] != unique_entities:
                raise ValueError("Document ID already has different entities; use a new revision ID")
            return
