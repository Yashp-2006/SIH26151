...
def load(accounts_path=None, pairs_path=None):
    try:
        from pramana.data import load_accounts, data_path
    except ModuleNotFoundError:
        from data import load_accounts, data_path
    accounts = {a["account_id"]: a for a in load_accounts(accounts_path)}
    with data_path(pairs_path, "pairs.csv").open(encoding="utf-8", newline="") as stream:
        pairs = list(csv.DictReader(stream))
    seen = set()
    for row in pairs:
        a, b = row.get("account_a"), row.get("account_b")
        if a not in accounts or b not in accounts or a == b:
            raise ValueError("Candidate pair requires two distinct known accounts")
