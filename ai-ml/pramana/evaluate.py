...
def load_key(path=None):
    key_path = path if path is not None else _resolve_file("answer_key.csv")
    with open(key_path, encoding="utf-8", newline="") as stream:
        rows = list(csv.DictReader(stream))
    key = {}
    for row in rows:
        account, operator = row.get("account_id"), row.get("operator_id")
        if not account or not operator or account in key:
            raise ValueError("Invalid or duplicate ground-truth account")
        key[account] = operator
    return key


def confusion(preds, truth):
    preds, truth = list(preds), list(truth)
    if len(preds) != len(truth):
        raise ValueError("Prediction and truth lengths differ")
    if any(type(v) not in (bool, int) or v not in (0, 1) for v in preds + truth):
        raise ValueError("Predictions and truth must be binary")
...
def run(lam=score_pramana.LAMBDA, tau=None, quiet=False, grouping=True, data_dir=None):
    ...
    rarity = RarityIndex(directory / "accounts.json", tau=TAU if tau is None else tau)
    key = load_key(directory / "answer_key.csv")
    if set(key) != set(accounts):
        raise ValueError("Ground truth and reference population do not match")
    ...
    for r in pairs:
        ...
        obs = obs_by_pair[pid]
        t = key[a] == key[b]
        if r.get("is_same_operator") != str(int(t)):
            raise ValueError("Pair label disagrees with quarantined ground truth")
    ...
    if not quiet:
        ...
        print(f"asserted params: lambda={lam}  tau={rarity.tau}  k_min="
...
def ablation_table():
    ...
    print("=" * 78)
    print(f"RESIDUAL FAILURES ({len(fp)} remaining false merges):")
