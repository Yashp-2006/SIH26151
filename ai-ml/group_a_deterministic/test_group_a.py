...
def check_extractors(gt: dict) -> int:
    print("\n=== extractors.py ===")
    failures = 0
...
def check_canonicalise_text(gt: dict) -> int:
...
def check_canonicalise_image(gt: dict) -> int:
...
def check_pgp_meta(gt: dict) -> int:
    print("\n=== pgp_meta.py ===")
    failures = 0
...
def main():
    ...
    total_failures = 0
    total_failures += check_extractors(gt)
    total_failures += check_canonicalise_text(gt)
    total_failures += check_canonicalise_image(gt)
    total_failures += check_pgp_meta(gt)
    total_failures += check_rarity(gt)
...
# Offline pytest regressions; the original remote validation remains in main().
def test_offline_extractors():
    results = extract_indicators("-----BEGIN PGP PUBLIC KEY BLOCK-----")
    assert "pgp_block_start" in {r["type"] for r in results}


def test_offline_text():
    assert are_near_duplicates("one two three", "one two three")
    assert canonicalise_text("one two three") == canonicalise_text("one two three")


def test_offline_image(tmp_path):
    from PIL import Image
    path = tmp_path / "fixture.png"
    Image.new("RGB", (16, 16), "red").save(path)
    assert images_are_near_duplicates(str(path), str(path))


def test_offline_invalid_pgp():
    assert "error" in parse_pgp_key("not a key")


def test_offline_rarity():
    assert compute_rarity("hub", {"hub": 15})[1] is True
    assert compute_rarity("unseen", {"hub": 15})[0] == float("inf")
